...
| 19 | **Semantic search toggle in Find** | verified (on-device embeddings) | keyword only | research → **M** | **Researched 2026-09-02 — [`semantic-search-research.md`](semantic-search-research.md).** Still open; the decision on offer: in-process CPU embeddings through `fastembed` (ONNX Runtime) running `BAAI/bge-small-en-v1.5` 8-bit (MIT, 384-d, ~34 MB), downloaded on first use from our own release with the updater's SHA-256 check rather than baked into the AppImage; flat per-document vector files under `.aquarius/semantic.nosync/<model-key>/` with a brute-force cosine scan (no SQLite in an iCloud folder); the index is per machine and per model because Apple's `NLEmbedding` vectors cannot be reproduced off a Mac, so the shared contract is about the two apps not clobbering each other, not about sharing vectors; an *Exact · By meaning* toggle in Find with a pandoc-style "needs a 35 MB model — Download" card; one MCP tool, `search_semantic`, that refuses structurally when the model is absent. Starts with a half-day CI spike on the ONNX Runtime link; `candle` is the named fallback. |
...
**Counts.** 2 open rows. Everything closed on 2026-08-30, in order: the
