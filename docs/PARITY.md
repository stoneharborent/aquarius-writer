...
| ~~8~~ | ~~**Manuscript management** (mark folder as manuscript, ManuscriptHome grid, status filter chips, front-matter section)~~ | verified | ✅ **done 2026-09-02** — Mark as Manuscript / Mark as Draft folder in the sidebar row's ⋯ menu (with an `MS` / `Draft` chip on the marked row), a ManuscriptHome grid of cards, status filter chips with per-status counts filtering the outline and the corkboard together, a FRONT MATTER section, a Working Draft pill, rail stats, and the corkboard synopsis edited in place | **M** | NOTES §30. The marking UI **closed the last of the four MCP inversions** the section below names: `toggle_manuscript_folder` / `toggle_draft_folder` are the same `vault::ops` functions behind both doors, through four new thin commands. Two tools went the other way in the same change — `set_active_draft` (new `ops` behaviour, so it shipped with its tool) and `list_manuscripts` (the read side of the home grid): **31 tools → 33**. Two things the Swift audit does not specify were decided and written down: **front matter is a file-name convention** — `Title Page.md` / `Dedication.md` / `Epigraph.md`, directly in the manuscript folder, case ignored — and it is deliberately **not** part of the chapter order (`tree::chapter_paths_in`), so a title page is never chapter one; and **ManuscriptHome sits behind the sidebar's Manuscript quick view**, while ⌘2 still goes straight to the outline. Pages is `words / 250` rounded up, in one place (`lib/manuscript.ts` + `ops::list_manuscripts`), because four surfaces quote it. Found and fixed on the way: a folder-backed draft was never made active, so a vault whose only cut came from `toggle_draft_folder` had no working draft at all (NOTES §30g). |
...
| ~~11~~ | ~~**Editable split editor**~~ | verified — two live documents, independent undo/save | ✅ **done 2026-08-31** — the secondary pane is a real editor: its own CodeMirror view, caret, scroll and undo stack, edits flowing through the *same* `editorStore` buffer, 800ms debounce and conflict guard as the primary. Draggable divider clamped at 320px a side and persisted; active-pane accent line; "Open in Split View" on the sidebar row's ⋯ menu; **Editable ↔ Reference** toggle on the pane's slim header, so Swift's read-only Reference pane is still there | **M** | `splitStore` gained `active`, `ratio` (persisted as `aquarius.split.ratio`) and a `SPLIT_MIN`; the editor column became a **grid** with the shared `shell/Splitter` in its own 1px track, so every pane width is a whole pixel (NOTES §1a). The toolbar now follows the **active** pane instead of always the primary (`useToolbarContext` takes a pane and an active flag), which is what makes ⌘B land in the half the caret is in — `formatBus` already keyed by path and needed no change. `readOnly` is a new mount-time prop on all three editors (`EditorState.readOnly` **and** `EditorView.editable`), and a read-only view deliberately does not register as a format target. **The same document cannot be edited in both panes** — the second copy is read-only and says so; two CodeMirror views over one buffer would each clobber the other's selection and undo on every keystroke (NOTES §25b). Viewers in the split stay viewers. NOTES §25. |
| ~~12~~ | ~~**Screenplay depth**: paged canvas with real page breaks, Title Page editor tab, scene drag-reorder, smart-type~~ · dual dialogue + revision marks **deferred** | verified | ✅ **mostly done 2026-08-31** — a real paged canvas on the industry point geometry, a Script / Title Page tab pair writing the Fountain title block, scene drag-reorder that rewrites the script through the same rules as the MCP tool, and character-name / scene-heading smart-type | **L** | **Shipped.** (a) **Paged canvas** — US-Letter sheets at 612×792pt mapped 1pt = 4/3px (CSS's own conversion; an 816×1056px page, a 16px line box, 54 body rows, every vertical length an exact integer multiple of the line at every zoom rung). CodeMirror keeps ONE continuous document: the sheets are a **repeating background** on a dedicated layer behind `.cm-content` (`::before` at `inset: 0`, so it is exactly the same box the gradient's origin always used) and each page break is a line decoration whose padding closes the sheet above it, so nothing is ever measured and the pages cannot drift off the text. Page numbers top-right from page 2, in the page's own face. The chrome is Final Draft's since 2026-08-31 — flat near-white paper with black ink on every theme, a crisp 1px edge, a 2px offset hairline instead of a drop shadow, and **no rule under a scene heading** (NOTES §27k). (b) **A line-accurate pagination engine** (`fountain-pages.ts`) replaced the old estimate, which double-counted element spacing on top of Fountain's own blank lines and ran long; the canvas, the footer badge and the print-preview overlay now all read the same page breaks. (c) **Title Page tab** — the six fields on a real sheet, written into the Fountain title block through the normal buffer and the normal conflict-guarded save; unknown keys (`Copyright:`, `Notes:`) round-trip in place; no frontmatter anywhere. (d) **Scene drag-reorder** with slide-into-place, rewriting the script via `fountain-scenes.ts` — a deliberate TS mirror of `src-tauri/src/vault/fountain.rs`, including its two hard-won rules (nothing above the first heading moves; a moved last scene gets the blank line a heading needs) — and going back in as a normal undoable buffer edit. (e) **Smart-type** — character names by recency on a cue line, and scene headings from the slugs the script already uses, on `@codemirror/autocomplete` like the wiki-link work. **Deferred, explicitly:** *dual dialogue* (side-by-side `^` blocks) and *revision marks* — see NOTES §27f for why dual dialogue is not a styling job here: two speakers sharing one band of rows breaks the one-source-line-is-N-rows model the whole paged canvas is built on, and wants a real column primitive. NOTES §27. |
...
| ~~18~~ | ~~**Terminal pane**~~ | verified — multi-session tabs, agent config, drag-file-for-path | ✅ **done 2026-08-31** — a real PTY per session in the right pane's third tab (⇧⌘J): named tabs with a connected dot, a plus and a close, the workflow's folder as the cwd, a configurable startup command, A−/A+ font sizing, and a file dragged out of the sidebar typing its absolute path | **M** | **This is the other half of the MCP server.** Settings prints a `claude mcp add` line; this is where the writer runs `claude` so that line means something — in the app, in the workflow's directory, no embedded model. Rust side: `src-tauri/src/pty/` on the `portable-pty` crate (forkpty / ConPTY behind one trait, so it is not a macOS-only feature the way SwiftTerm made it), five commands, output over a `pty://output` event and a `pty://exit` event, **two threads per session — a reader and a waiter, and the waiter is the zombie reaper**. Front end: `@xterm/xterm` + `@xterm/addon-fit`, instances held in a module-level registry *outside React* so collapsing the pane does not kill a running `claude` (only closing the tab does). **Session configs persist — names, font sizes, startup commands. Live PTYs never do**, and a relaunched app opens its tabs cold with a Launch button rather than showing a dead prompt that eats keystrokes. **No capability change**: the PTY lives in our own Rust commands, so nothing was granted to `tauri-plugin-shell` and no scope widened. **Not ported from Swift:** pinning and the model/effort chip — both describe an embedded agent this app deliberately does not have. NOTES §26. |
| ~~19~~ | ~~**Semantic search toggle in Find**~~ | verified (on-device embeddings) | ✅ **done 2026-09-02** — an *Exact · By meaning* toggle in the Find sheet, backed by `BAAI/bge-small-en-v1.5` (8-bit ONNX, MIT, 384-d) running in-process on the CPU through `fastembed`; a Settings → Search tab; and the `search_semantic` MCP tool | **M** | Built to [`semantic-search-research.md`](semantic-search-research.md) as written. **The spike passed first time on both runners** — pyke's prebuilt ONNX Runtime 1.28 links **statically**, so nothing is bundled and no rpath is needed; `candle` was never called for. The AppImage grew 84.1 → 93.0 MB and the Mac zip 10.3 → 19.1 MB, and `minimumSystemVersion` had to be raised to 13.3 (ORT's own floor; Tauri's default of 10.13 would have shipped an app that could not start). The model is **not** in the app — it comes from this repo's `models-v1` release on a human click, every file checked against a digest compiled into the binary. Index: flat per-document `.vec` files under `.aquarius/semantic.nosync/<model-key>/`, brute-force cosine, rebuilt on any key mismatch, re-embedded per document on a stamp mismatch, re-keyed rather than re-embedded on a rename. Both doors are wired: saving in the app and every mutating MCP tool keep it current, and neither is on the keystroke path. NOTES §32. **Deferred:** an optional GPU backend behind the same `Embedder` trait (Ollama / `llama-server`), and the Swift app adopting the same folder — both named in §6 of the research doc as later work, neither needed for parity. |
...
**Counts.** **No open rows.** Row 19 — search by meaning — closed on
2026-09-02 and it was the last one; the table's only unstruck heading now is
row 2, which was finished on 2026-08-30 and never had its tildes added.
Everything closed on 2026-08-30, in order: the
...
trash's confirm-first alignment shipped alongside. Then the two mediums that
close the wave: row 11 (the editable split) and **row 18 (the terminal pane)**,
which was deferred on purpose until the layout settled and had a slot to live
in — it now does, as the right pane's third tab. Then **row 12, the last large
one**: screenplay depth — a real paged canvas, the Title Page tab, scene
drag-reorder and smart-type, with dual dialogue and revision marks deferred on
the record (NOTES §27f). **Every small row is closed, and so is every large
one.** What is left is 1 medium (8 manuscript management), 1 research (19
semantic search), and the two screenplay deferrals.
...
11. ~~**Screenplay depth** (row 12, L)~~ ✅ **done 2026-08-31** — the paged
    canvas is arithmetic, not measurement: 54 rows a page on the point
    geometry, sheets painted as a repeating background, and a page-break
    decoration whose padding closes the sheet above it. Shipped with it: a
    line-accurate pagination engine shared by the canvas, the footer and the
    print preview; the Script / Title Page tab pair; scene drag-reorder
    mirroring the Rust `reorder_scenes` the MCP tool calls; and character /
    scene-heading smart-type. Dual dialogue and revision marks deferred
    (NOTES §27f). And ~~**editable split** (row 11, M)~~ ✅
    **done 2026-08-31** — two real editors side by side over the same buffer
    machinery, a clamped and persisted divider, and the active pane owning the
    top bar's toolbar. Swift's read-only Reference pane survives as a mode on
    the same split rather than a second mechanism. NOTES §25.
...
14. ~~**Terminal pane** (row 18, M)~~ ✅ **done 2026-08-31** — the home the
    Wave 1 layout was supposed to give it turned out to be the right pane's
    third tab, beside Comments and Versions (⇧⌘J), which is Swift's own
    one-slot model. Real PTYs in Rust (`portable-pty`), xterm.js in the
    webview, and the bring-your-own-agent flow the MCP server was waiting for.
    NOTES §26.
15. ~~**Semantic search** (row 19, M)~~ ✅ **done 2026-09-02** — researched
    first, then built to that document. The half-day spike it was gated on
    passed on both CI runners at the first attempt: ONNX Runtime links
    statically, so there is no dylib to bundle and `candle`, the named
    fallback, was never needed. Exact · By meaning in the Find sheet, a
    35 MB model downloaded on a click and checked byte for byte, an index of
    flat vector files the app treats as a disposable cache, and one MCP tool
    that refuses honestly when the model is not there. NOTES §32.

    **This closes the parity table.** Every row the Swift audit raised is now
    either shipped, decided against with the reason written down, or — in the
    two cases the platform refuses (row 15's folder drop, row 16's watched
    hardware) — degraded to something honest and documented as such.
...
**Row 8 closed that gap in the sidebar** (2026-09-02). `toggle_manuscript_folder`
and `toggle_draft_folder` are in the tree row's ⋯ menu now, calling the same two
`ops` functions the tools call, so two of the four deliberate inversions are
gone. `take_snapshot` under a client-chosen name and `diff_version` between two
saved versions remain — both still doing by one route what the Versions panel
does by another. And the rule ran the *other* way again in the same change: the
Working Draft pill and the ManuscriptHome grid are things the window could do
and an agent could not, so `set_active_draft` and `list_manuscripts` shipped
beside them rather than after them. Thirty-three tools.
